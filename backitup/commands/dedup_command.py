...
def dedup_against_old_backups(new_backup: BackupLogIter, old_backups: List[BackupLogIter], opts: DedupOptions, runner):
    ...
    # dups.
    batch_of_edits = batched_as_dict(new_backup, opts.batch_size)

    for (entries_dict, min_hash, max_hash) in batch_of_edits:
        for old_backup_log in old_backups:
            while True:
                old_log_entry = old_backup_log.peek()
                if not old_log_entry:
                    break
                old_file_hash = old_log_entry.hash
                if old_file_hash > max_hash:
                    break
                if old_file_hash >= min_hash:
                    dups = entries_dict.pop(dedup_key(old_log_entry), None)
                    if dups:
                        res = find_content_duplicates_of(
                            old_log_entry.path, [dup.path for dup in dups])
